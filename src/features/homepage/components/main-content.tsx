...
export interface HomepageData {
  navigation_bar: HomepageNavigationItem[];
...
export default function MainContent({ data }: MainContentProps) {
  ...
  const mappedProducts: FeaturedProductItem[] = (data?.products ?? []).map((p, idx) => ({
    ...
    price: typeof p.price === "number" ? p.price : "Liên hệ",
  }));
    ...
    <main className="flex min-h-screen flex-col">
      ...
      <footer className="bg-gray-900 py-12 text-gray-300">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4">
            <div>
              <h3 className="mb-4 text-xl font-bold text-white">{TEXT.footer.aboutTitle}</h3>
              <p className="mb-4">{TEXT.footer.aboutDescription}</p>
              <SocialLinks />
            </div>

            <div>
              <h3 className="mb-4 text-xl font-bold text-white">{TEXT.footer.quickLinksTitle}</h3>
            ...
            <div>
              <h3 className="mb-4 text-xl font-bold text-white">{TEXT.footer.servicesTitle}</h3>
              <ul className="space-y-2">
                <li>
                  <Link href="#" className="hover:text-green-400">
            ...
            <div>
              <h3 className="mb-4 text-xl font-bold text-white">{TEXT.footer.contactTitle}</h3>
              <address className="not-italic">
                <p className="mb-2">{footer?.address || TEXT.footer.contact.address}</p>
                <p className="mb-2">Email: {footer?.email || TEXT.footer.contact.email}</p>
                <p className="mb-2">Điện thoại: {footer?.phone || TEXT.footer.contact.phone}</p>
              </address>
            </div>
          </div>
          ...
          <div className="mt-8 border-t border-gray-800 pt-8 text-center">
            <p>
              &copy; {new Date().getFullYear()} {footer?.short_name || footer?.vi_name || TEXT.footer.copyright}
            </p>
          </div>
        </div>
      </footer>
